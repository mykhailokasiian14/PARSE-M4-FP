...
public class Main {
    ...
    private List<City> fetchData(Main main) {
        try (Session session = main.sessionFactory.getCurrentSession()) {
            ...
            // прогріваємо кеш країн, щоб не було N+1 проблем
    ...
    private List<CityCountry> transformData(List<City> cities) {
        return cities.stream().map(city -> {
            ...
            res.setContinent(String.valueOf(country.getContinent()));
    ...
    public static void main(String[] args) {
        ...
        // 1. отримуємо всі міста з MySQL
        List<City> allCities = main.fetchData(main);

        // 2. трансформуємо у DTO
        List<CityCountry> preparedData = main.transformData(allCities);

        // 3. кешуємо в Redis
        main.pushToRedis(preparedData);

        // 4. закриваємо поточну сесію Hibernate (щоб не брати з кешу)
        main.sessionFactory.getCurrentSession().close();

        // 5. перевірка — 10 випадкових id
